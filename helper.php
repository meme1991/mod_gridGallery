...
class ModGridGallery
{
	...
	public static function category($catid, $image = NULL){
		...
		$query->select($db->quoteName(array('p.title', 'p.filename', 'p.catid', 'pc.alias', 'pc.image')));
	 ...
	 * @return array $image
	 */
	public static function categories($catid){
		...
		$query->select($db->quoteName('pc.title', 'category_title'));
		$query->select($db->quoteName('pc.image_id', 'img_cover'));
		$query->from($db->quoteName('#__phocagallery', 'p'));
		$query->join('INNER', $db->quoteName('#__phocagallery_categories', 'pc') . ' ON (' . $db->quoteName('p.catid') . ' = ' . $db->quoteName('pc.id') . ')');
		$query->where($queryCatid);
		$query->where($db->quoteName('p.published') . ' = 1' );
		$query->group($db->quoteName('p.catid'));
		$query->order('pc.title DESC');
	 ...
	 * @return array $image
	 */
	public static function categories_only(){
